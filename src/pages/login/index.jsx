...
const { Tab, UserName, Password, Mobile, SmsCaptcha, Captcha, Submit } = LoginComponents;
...
class Login extends Component {
  ...
  handleSubmit = (err, values) => {
    ...
    if (!err) {
      const { dispatch } = this.props;
      const { userLogin: { captchaID } } = this.props;
      values.password = md5(values.password);
      dispatch({
        type: 'login/login',
        payload: { ...values, type, captchaId: captchaID },
        error: () => {
          const { setFieldsValue } = this.loginForm;
          setFieldsValue({ captcha_code: '' })
        }
      });
    }
  };
  ...
  render() {
    ...
    const { status, type: loginType, captchaID } = userLogin;
      ...
      <div className={styles.main}>
        ...
        >
          ...
          >
            ...
            { captchaID ? <Captcha
              name="captcha_code"
              image={`${API.Public.Login.ResCaptcha}?id=${captchaID}`}
              placeholder={formatMessage({
          ...
          >
            ...
            <SmsCaptcha
              ...
              countDown={120}
              onGetCaptcha={this.onGetSmsCaptcha}
